...
var A, API, BUILD, Base, Client, EQ, FILE, FN, FNon, FO, Flexbase, LG, N, NN, O, PORT_CMS, PORT_WEB_SOCKET, PROD, ST_EMAIL, ST_TEXT, Server_UTDispatcher, TRACE_RECEIVED, TRACE_SENT, T_API, T_CMS, URL, UT, after, app, app2, argsNode, aws, beg, bodyParser, bulkForm, clientMap, contextFactory, cookieParser, down, end, express, expressWs, fork, form, h, hhh, http, http2, indent, lg, linkType, logBase, logError, logcatch, msgInsert, multer, multerS3, nbr, nodemailer, pair, pathWrite, peter, peter__id, plus, port, proxyMap, qqq, raw, recentsBuild, recentsWrite, reqCounter, rnd, sendCounter, server, server_UTDispatcher, skillSelectAllEditor, skillTags, sql, stripHTTP, throttle, tipsWrite, top, trace, twilio, upload, uptimeBeg, util, wsMap;
...
Server_UTDispatcher = class Server_UTDispatcher extends Base {
  constructor() {
    super();
  }
  ...
  listen() {
    ...
    argsNode.forEach(function(item) {
      return console.log(item);
    });
    return setTimeout(() => {
      var child;
      child = fork('./server_UT.js', ["peter", "alvin"], {
        stdio: 'pipe'
      });
      return this.log("forked", child);
    }, 1000);
  }
...
PORT_CMS = PROD ? 3333 : 3344;

PORT_WEB_SOCKET = PROD ? 3355 : 3888;

BUILD = `${(PROD ? "PROD" : "DEV")} 18-05-15.1 (0.0.95)`;
...
LG = function(file, s, v) {
  ...
  return console.log(`${NN(date.getMinutes())}:${NN(date.getSeconds())} ${BUILD} ${s}${vPart}`);
};
...
logError = function(s, ex) {
  return LG(FILE, `ERROR: ${s}`, ex);
};

logcatch = function(s, ex) {
  return LG(FILE, `CATCH: ${s}`, ex);
};

EQ = function(v1, v2) {
  return console.log(`COMP: ${v1} vs. ${v2} (${typeof v1}) vs (${typeof v2}) ${(v1 === v2 ? "YES-MATCH" : "NO-MATCH")}`);
};
...
app.ws('/', function(ws, req) {
  ...
  return ws.on('message', function(json) {
    var _, c, flexbase, myObj, po, sendAck;
    if (TRACE_RECEIVED) {
      lg("received", json);
    }
    po = JSON.parse(json);
    sendAck = function(errorMsgOpt, rvpo) {
      var _, pn, pv;
      _ = {
        target: "flexbase",
        cmd: `${po.cmd}-ack`,
      ...
      return ws.send(JSON.stringify(_));
    };
    //		lg "po.cmd=#{po.cmd}"
    switch (po.cmd) {
      case "test":
      ...
      case "c-fb-register":
        //				lg "c-fb-register", po
        if (po.username === "remoteportal" && po.password === "1234") {
          //					lg "INSIDE"
          flexbase = new Flexbase(ws);
          return flexbase.create({
          ...
          return sendAck("email already registered");
        }
        break;
      case "c-fb-login":
        //				lg "c-fb-login", po
        if (po.username === "remoteportal" && po.password === "1234") {
          //					lg "INSIDE"
          flexbase = new Flexbase(ws);
          return flexbase.get(peter__id).then((user) => {
            return sendAck(null, {
              user: user
            });
          }).catch((ex) => {
          ...
          return sendAck("account not found");
        }
        break;
      case "c-fb-object-insert":
        if (!(po.fo.__id < 0)) {
          throw "!insert";
        }
        c = contextFactory(null, null, 1, "c-fb-object-insert", null, null, false);
        if (po.fo.fname === 'Deanna') {
          //					lg "FOUND DEANNA; caching FO"
          FO = po.fo;
        }
        return c.conn.query('call objectInsert(?,?,?,?)', [1, 1, 'hello', JSON.stringify(po.fo)], function(err, rsets) {
          if (err) {
            return c.ex(err);
          } else {
            ...
            if (po.fo.__cn === "SmileSpeak.Delivery") {
              lg("SMILE DELIV!");
              flexbase = new Flexbase(ws);
              flexbase.get(po.fo.idTo).then((_to) => {
                return lg(`delivery of ${po.fo.idRecording} from ${po.fo.idFrom} to ${po.fo.idTo}`, po.fo);
              }).catch((ex) => {
                return sendAck(ex);
              });
            }
      ...
      case "c-fb-object-update":
        c = contextFactory(null, null, 1, "c-fb-object-update", null, null, false);
        if (po.fo.__id === 0) {
          throw "!update";
        }
        return c.conn.query("call objectDataUpdate(?,?)", [po.fo.__id, JSON.stringify(po.fo)], function(err, rsets) {
          if (err) {
            return c.ex(err);
          } else {
