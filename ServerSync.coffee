###
ServerSync - communication persistence
...
#ifdef ut
class ServerSyncUT extends UT
	run: ->
		@a "client/server send object back and forth", {timeout:3000}, (ut) ->
			...
			class _Server extends ServerSync
				onReceiveSync: (mgr, po) ->
#					@log "onReceiveSync", po
					mgr.reply answer:"rain"

			class _Client extends ClientSync
			...
			.then =>
				...
				.then (po) =>
#					@log "sendSync completed", po, true
					@eq po.o.answer, "rain"
					cleanUp()
					ut.resolve()
				.catch (ex) =>
...
#		@log "Mgr.reply", po
		...
		#BACKED_OUT: because of client issue
#		@ws.send JSON.stringify po, (err) =>
#			if err
#				#TODO: log multiple objects before options
#				@logTransient "ws.send", err
		@ws.send JSON.stringify po
			



module.exports = class ServerSync extends Server
	constructor: (@port, @directory) ->
		super()
#		@log "ServerSync: port=#{@port} directory=#{@directory}"
...
#				resolve "peter"
	...
	onReceive: (ws, po) ->
#		@log "onReceive [DEFAULT]", po
#		@store.write "test", a:"first"

		unless po.cmd is "c-to-s"
			@logAssert "DO.cmd: expecting='c-to-s' got='#{DO.cmd}'", DO, true
			return
