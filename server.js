...
var A, API, Base, Client, FN, FNon, FO, Flexbase, N, O, ST_EMAIL, ST_TEXT, Server, ServerUT, T_API, T_CMS, URL, UT, after, app, app2, aws, beg, bodyParser, bulkForm, clientMap, contextFactory, cookieParser, down, end, expressWs, form, h, hhh, http, http2, indent, linkType, msgInsert, multer, multerS3, nbr, nodemailer, pair, pathWrite, peter, plus, port, proxyMap, qqq, raw, recentsBuild, recentsWrite, reqCounter, rnd, sendCounter, server, skillSelectAllEditor, skillTags, sql, stripHTTP, throttle, tipsWrite, top, trace, twilio, upload, util, wsMap;
...
ServerUT = class ServerUT extends UT {
  run() {
    return this.s("ServerUT", function() {
      this.a("listen/unlisten", function(ut) {
        var server;
      ...
      this.a("listen with client with no packets sent", function(ut) {
        var server;
        server = new Server(3366);
        server.listen(true);
        this.log("try to connect");
        this.client = new Client("ws://localhost:3366");
        this.client.listen(true).then(() => {
      ...
      this.a("listen with client with packets sent", function(ut) {
        var _Client, _Server, client, server;
        _Server = class _Server extends Server {
          onReceive(ws, o) {
            this.log("onReceive", o);
            ws.send(JSON.stringify({
              server_says: "back at cha" //H: abstract so not have to stringify?
            }));
            return ut.resolve();
          }

        };
        _Client = class _Client extends Client {
          onReceive(o) {
            this.log("onReceive", o);
            //						@log "this", this, true
      ...
      return this.a("two parallel", function(ut) {
        return ut.resolve();
      });
    });
  }
...
//endif
module.exports = Server = class Server extends Base {
  constructor(port1) {
    var express, expressWs;
    super();
    this.port = port1;
    express = require('express');
    this.app = express();
    expressWs = require('express-ws')(this.app);
    ...
    this.app.ws('/', (ws, req) => {
      if (trace.SOCKET_CONNECTED) {
        this.log("client connected at /");
      }
      return ws.on('message', (json) => {
        var ex, o;
        if (trace.SOCKET_RECEIVED) {
          this.log("received", json, true);
        }
        try {
          o = JSON.parse(json);
        } catch (error) {
          ex = error;
          this.logTransient("JSON.parse", ex);
        }
        return this.onReceive(ws, o);
      });
    });
  }
  ...
  listen(bAlive) {
    if (trace.SOCKET_LISTEN) {
      ...
      this.log(`listen ${bAlive} port=${this.port}`);
    }
    if (bAlive) {
      ...
      return this.server.close((err) => {
        if (err) {
          return this.logErr("server.close", err);
        } else {
          if (trace.SOCKET_CLOSE) {
            return this.log("server closed");
          }
        }
      });
    }
  ...
  send(ws, o) {
    if (trace.SOCKET_SENT) {
      return this.log("send", o);
    }
  }
