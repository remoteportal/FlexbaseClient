...
-
*/
var A, API, BUILD, Base, Client, ClientSync, FN, FNon, FO, Flexbase, Mgr, N, O, PORT_CMS, PORT_WEB_SOCKET, PROD, ST_EMAIL, ST_TEXT, Server, ServerSync, ServerSyncUT, Store, TRACE_RECEIVED, TRACE_SENT, T_API, T_CMS, URL, UT, after, app, app2, aws, beg, bodyParser, bulkForm, clientMap, contextFactory, cookieParser, down, end, expressWs, form, h, hhh, http, http2, indent, linkType, logBase, msgInsert, multer, multerS3, nbr, nodemailer, pair, pathWrite, peter, plus, port, proxyMap, qqq, raw, recentsBuild, recentsWrite, reqCounter, rnd, sendCounter, server, skillSelectAllEditor, skillTags, sql, stripHTTP, throttle, tipsWrite, top, trace, twilio, upload, uptimeBeg, util, wsMap;
...
//ifdef ut
ServerSyncUT = class ServerSyncUT extends UT {
  run() {
    return this.a("client/server send object back and forth", {
      timeout: 3000
    }, function(ut) {
      var _Client, _Server, cleanUp, client, port, server;
      client = null; // establish scope
      port = this.testHub.portNext();
      cleanUp = () => {
        server.listen(false);
        return client.listen(false);
      };
      _Server = class _Server extends ServerSync {
      ...
      _Client = class _Client extends ClientSync {};
      server = new _Server(port, `${this.testHub.c.directory}/ServerSyncUT_server`);
      return server.listen(true).then(() => {
        this.log("create client and connect");
        client = new _Client(`ws://localhost:${port}`, `${this.testHub.c.directory}/ServerSyncUT_client`);
        return client.listen(true).then(() => {
          ...
          this.eq(po.o.answer, "rain");
          cleanUp();
          return ut.resolve();
        }).catch((ex) => {
          this.logCatch("server.listen.then.client.listen chain", ex);
          cleanUp();
          return ut.reject(ex);
        });
      }).catch((ex) => {
        this.logCatch("server.listen", ex);
        return ut.reject(ex);
      });
    });
  }
...
Mgr = class Mgr extends Base {
  ...
  reply(o) {
    ...
    return this.ws.send(JSON.stringify(po));
  }

};

module.exports = ServerSync = class ServerSync extends Server {
  ...
  onReceive(ws, po) {
    ...
    if (po.cmd !== "c-to-s") {
      this.logAssert(`DO.cmd: expecting='c-to-s' got='${DO.cmd}'`, DO, true);
      return;
    }
    mgr = new Mgr(ws, po);
