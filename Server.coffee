...
class ServerUT extends UT
	run: ->
		@s "ServerUT", ->
			@a "listen/unlisten", (ut) ->
				server = new Server 3366
				server.listen true
				...
				,
					3000
			@a "listen with client with no packets sent", (ut) ->
				server = new Server 3366
				server.listen true

				@log "try to connect"
				@client = new Client "ws://localhost:3366"
				@client.listen true
				.then =>
					@log "connected"
				.catch (ex) =>
...
#					@log "timeout"
				...
				,
					3000
			@a "listen with client with packets sent", (ut) ->
				class _Server extends Server
					onReceive: (ws, o) ->
						@log "onReceive", o
						ws.send JSON.stringify server_says:"back at cha"		#H: abstract so not have to stringify?
						ut.resolve()
				...
				class _Client extends Client
					onReceive: (o) ->
						@log "onReceive", o
#						@log "this", this, true
						ut.eq o.server_says, "back at cha"
						server.listen false
						client.listen false
						ut.resolve()

				server = new _Server 3366
				server.listen true

				@log "try to connect"
				client = new _Client "ws://localhost:3366"
				client.listen true
				.then =>
					@log "connected"
					client.send example_object:"example value"
				.catch (ex) =>
#					O.LOG ex
					if ex is "Invalid URL"
#						@log "correct"
...
module.exports = class Server extends Base
	constructor: (@port) ->
		...
		#TODO: simplify this as I have no idea what it does:
...
#			@log 'get route', req.testing
			res.end()
		@app.ws '/', (ws, req) =>
			@log "client connected at /"															if trace.SOCKET_CONNECTED
			ws.on 'message', (json) =>
				@log "received", json, true															if trace.SOCKET_RECEIVED

				try
					o = JSON.parse json
				catch ex
					@logTransient "JSON.parse", ex

				@onReceive ws, o

	listen: (bAlive) ->
		#TODO: return promise for BOTH
		@log "listen #{bAlive} port=#{@port}"														if trace.SOCKET_LISTEN
		...
		if bAlive
			...
			@server.close (err) =>
				if err
					@logErr "server.close", err
				else
					@log "server closed"															if trace.SOCKET_CLOSE

	onReceive: (o) ->
#		@log "onReceive [DEFAULT]", o, true

	send: (ws, o) ->
		@log "send", o																				if trace.SOCKET_SENT
		#H
		
		
#if ut
	@s_ut: -> new ServerUT().run()
#endif
